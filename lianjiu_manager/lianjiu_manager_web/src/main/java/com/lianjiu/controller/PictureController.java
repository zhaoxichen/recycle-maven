...
public class PictureController {
	...
	/**
	 * 
	 * zhaoxi 2017年10月11日 descrption:图片服务器迁移
	 * 
	 * @param oldPictureIp
	 * @param newPictureIp
	 * @return LianjiuResult
	 */
	@RequestMapping(value = "/picture/{oldPictureIp}/{newPictureIp}/transfer", method = RequestMethod.GET)
	@ResponseBody
	public LianjiuResult pictureTransfer(@PathVariable String oldPictureIp, @PathVariable String newPictureIp) {
		if (Util.isEmpty(oldPictureIp)) {
			return LianjiuResult.build(401, "指定要更新的图片url");
		}
		if (Util.isEmpty(newPictureIp)) {
			return LianjiuResult.build(402, "指定要更新的图片url");
		}
		System.out.println(oldPictureIp + "----->" + newPictureIp);
		return pictureService.updatePictureServer(oldPictureIp, newPictureIp);
	}
}
