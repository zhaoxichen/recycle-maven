...
import com.lianjiu.payment.config.DatetimeUtil;
...
public class aliPayAppController {
	 ...
	 * @param mercid
	 *            商品id
	 * @param callback:jsonp参数的名字
	 */
	@RequestMapping(value = "/payment", method = RequestMethod.POST)
	@ResponseBody
	public synchronized LianjiuResult orderPay(HttpServletRequest request, HttpServletResponse response, String cashnum,
			String userId, String outTradeNo) {
		loggerAlipay.info("支付下订单 android:androidPayment");
		try {
			request.setCharacterEncoding("utf-8");
			response.setCharacterEncoding("utf-8");
			if (StringUtil.hasEmpty(cashnum, userId, outTradeNo)) {
				return LianjiuResult.build(404, "请输入具体参数");
			}
			...
			e1.printStackTrace();
			return LianjiuResult.build(501, "出现异常");
		}
	 ...
	 * @param callback:jsonp参数的名字
	 */
	@RequestMapping(value = "/paymentJP", method = RequestMethod.POST)
	@ResponseBody
	public synchronized LianjiuResult orderPayJP(HttpServletRequest request, HttpServletResponse response,
			String cashnum, String userId, String outTradeNo) {
		loggerAlipay.info("支付下订单 android:androidPayment");
		try {
			request.setCharacterEncoding("utf-8");
			response.setCharacterEncoding("utf-8");
	 ...
	 * @param callback:jsonp参数的名字
	 */
	@RequestMapping(value = "/iosPayment", method = RequestMethod.POST)
	@ResponseBody
	public synchronized LianjiuResult iosOrderPay(HttpServletRequest request, HttpServletResponse response,
			String cashnum, String userId, String outTradeNo) {
		...
		try {
			request.setCharacterEncoding("utf-8");
			response.setCharacterEncoding("utf-8");
			if (StringUtil.hasEmpty(cashnum, userId)) {
				return LianjiuResult.build(404, "请输入具体参数");
			}
			String ePrice = ordersExcellentMapper.slectPriceByOrderNo(outTradeNo);
			String fPrice = ordersFacefaceDetailsMapper.slectPriceByOrderNo(outTradeNo);
	 ...
	 * @param callback:jsonp参数的名字
	 */
	@RequestMapping(value = "/iosPaymentJP", method = RequestMethod.POST)
	@ResponseBody
	public synchronized LianjiuResult iosOrderPayJP(HttpServletRequest request, HttpServletResponse response,
			String cashnum, String userId, String outTradeNo) {
		loggerAlipay.info("支付下订单 ios:iosPayment");
		if (StringUtil.hasEmpty(cashnum, userId, outTradeNo)) {
			throw new RuntimeException("参数不能为空");
	...
	public void orderPayQuery(HttpServletRequest request, HttpServletResponse response, String tradeno, String orderno,
			String callback) {
		loggerAlipay.info("[/order/pay/query]");
	...
	@ResponseBody
	public synchronized void orderPayNotify(HttpServletRequest request, HttpServletResponse response) {
		loggerAlipay.info("订单支付微信服务器异步通知:[/order/pay/notify]");
	...
	public synchronized LianjiuResult orderPayRefund(HttpServletRequest request, HttpServletResponse response,
			String tradeno, String orderno, double amount) {
		loggerAlipay.info("订单退款:[/pay/refund]");
		if (StringUtil.isEmpty(tradeno) && StringUtil.isEmpty(orderno)) {
	...
	public synchronized void orderPayRefundQuery(HttpServletRequest request, HttpServletResponse response,
			String orderno, String tradeno, String callback) {
		loggerAlipay.info("[/pay/refund/query]");
		if (StringUtil.isEmpty(orderno) && StringUtil.isEmpty(tradeno)) {
			WebUtil.response(response, WebUtil.packJsonp(callback, JSON.toJSONString(
					new JsonResult(-1, "商家订单号或支付宝订单号不能为空", new ResponseData()), SerializerFeatureUtil.FEATURES)));
		}
