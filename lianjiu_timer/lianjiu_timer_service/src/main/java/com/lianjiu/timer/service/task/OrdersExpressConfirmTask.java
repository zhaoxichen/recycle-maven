...
public class OrdersExpressConfirmTask implements Runnable {
	...
	public OrdersExpressConfirmTask(OrdersExpressMapper ordersExpressMapper, JedisClient jedisClient) {
		this.ordersExpressMapper = ordersExpressMapper;
	...
	public void run() {
		...
		while (it.hasNext()) {
			...
			ttl = jedisClient.ttl(GlobalValueJedis.ORDERS_EXPRESS_CONFIRM_COUNT_DOWN_PRICE + ordersId);
			System.out.println(ordersId + "快递订单确认价格剩余时间-->" + ttl);
			if (-2 == ttl) {
				...
				loggerVisit.info(ordersIdKey + "，快递回收订单，确认价格");
		 ...
		 */
		if (isExpre) {
			ordersExpressMapper.ordersAutoCancel(ordersIdList);
		}
	}
