...
public class OrdersFurnitureTask implements Runnable {
	...
	public OrdersFurnitureTask(OrdersFurnitureMapper ordersFurnitureMapper, JedisClient jedisClient) {
		this.ordersFurnitureMapper = ordersFurnitureMapper;
	...
	public void run() {
		...
		while (it.hasNext()) {
			...
			ttl = jedisClient.ttl(GlobalValueJedis.ORDERS_FURNITURE_COUNT_DOWN_PRICE + ordersId);
			System.out.println(ordersId + "家具处理订单确认支付剩余时间-->" + ttl);
			if (-2 == ttl) {
				...
				loggerVisit.info(ordersIdKey + "家具处理订单，确认支付");
		 ...
		 */
		if (isExpre) {
			ordersFurnitureMapper.ordersAutoCancel(ordersIdList);
		}
	}
